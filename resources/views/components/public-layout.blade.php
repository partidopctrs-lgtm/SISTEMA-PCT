...
<body class="antialiased">
    <div class="min-h-screen flex flex-col" x-data="{ mobileMenuOpen: false }">
        <!-- Navbar -->
        <nav class="glass sticky top-0 z-50" x-data="{ projDropOpen: false }">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center h-20">

                    <!-- Logo -->
                    <a href="{{ route('home') }}" class="flex items-center space-x-2 flex-shrink-0">
                        <img src="{{ asset('logo.png') }}" alt="PCT Logo" class="h-10 w-auto">
                        <span class="hidden lg:block text-lg font-bold text-pct-blue tracking-tight">PCT</span>
                    </a>
                    ...
                    <!-- Desktop CTA buttons (md+) -->
                    <div class="hidden md:flex items-center space-x-2 flex-shrink-0">
                        <a href="{{ route('login') }}" class="px-3 py-2 text-sm text-pct-blue font-bold hover:underline transition-colors">Entrar</a>
                        <a href="{{ route('register.index') }}" class="px-4 py-2 bg-pct-blue text-white text-sm font-bold rounded-full hover:bg-blue-900 transition-all shadow-lg shadow-blue-900/20 whitespace-nowrap">Cadastre-se</a>
                    </div>

                    <!-- Hamburger (somente mobile) -->
                    <div class="flex md:hidden items-center">
                        <button @click="mobileMenuOpen = !mobileMenuOpen" class="text-pct-blue p-2 rounded-lg hover:bg-slate-50 transition-colors">
